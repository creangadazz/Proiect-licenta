            ...
            if (isset( $_SESSION['hotel_cart'])){
             ...
             $count_cart = count($_SESSION['hotel_cart']);
                ...
                for ($i=0; $i < $count_cart  ; $i++) {  
                    ...
                    $query = "SELECT * FROM `tblroom` r ,`tblaccomodation` a WHERE r.`ACCOMID`=a.`ACCOMID` AND ROOMID=" . $_SESSION['hotel_cart'][$i]['hotelroomid'];
                     $mydb->setQuery($query);
                     $cur = $mydb->loadResultList(); 
                      foreach ($cur as $result) { 
                        ...
                        echo '<td>'.date_format(date_create( $_SESSION['hotel_cart'][$i]['hotelcheckin']),"m/d/Y").'</td>';
                        echo '<td>'.date_format(date_create( $_SESSION['hotel_cart'][$i]['hotelcheckout']),"m/d/Y").'</td>';
                        echo '<td > &euro;'. $result->PRICE.'
                          <input type="hidden" value="'.$result->PRICE.'"  name="roomprice'.$_SESSION['hotel_cart'][$i]['hotelroomid'].'" id="roomprice'.$_SESSION['hotel_cart'][$i]['hotelroomid'].'"/>

                        </td>'; 
                        echo '<td><input style="border:0px" readonly type="number" value="'.$_SESSION['hotel_cart'][$i]['hotelday'].'" id="day'.$_SESSION['hotel_cart'][$i]['hotelroomid'].'" name="day'.$_SESSION['hotel_cart'][$i]['hotelroomid'].'" /></td>';
                        
                        echo  '<input type="hidden"  name="MealPrice'.$_SESSION['hotel_cart'][$i]['hotelroomid'].'" id="MealPrice'.$_SESSION['hotel_cart'][$i]['hotelroomid'].'"/>';
                        echo '</td>';
                        echo '<td>$<output id="TotAmount'.$_SESSION['hotel_cart'][$i]['hotelroomid'].'" >'.$_SESSION['hotel_cart'][$i]['hotelroomprice'].'</output></td>';
                        echo '<td ><a href="index.php?view=processcart&id='.$result->ROOMID.'">Remove</td>';
 
                      } 


                      $payable += $_SESSION['hotel_cart'][$i]['hotelroomprice'] ;
